...
from ontology_processor_ofn import process_ofn_files, parse_ofn_file
...
def main():
    """Main entry point for OFN (OWL Functional Syntax) → MkDocs + ODM diagrams."""
    log.debug("Starting ofn2md.py (OFN + SHACL support)")
    ...
    if len(sys.argv) != 1:
        print("Usage: python ofn2md.py [--create-missing | -c] [--dev | -dev] [--dev-map PATH]")
    ...
    ofn_files = [os.path.join(docs_dir, f) for f in os.listdir(docs_dir)
                 if f.lower().endswith(".ofn")]
    if not ofn_files:
    ...
    # === 1. Load ALL OFN files into one unified graph ===
    try:
        g, ns, prefix_map, all_classes, local_classes, prop_map, datatype_map = process_ofn_files(
            ofn_files, errors, dev_map=dev_map
        )
    except Exception as e:
        log.error(f"Failed to process OFN files: {e}")
        sys.exit(1)
    ...
    for ofn_path in ofn_files:
        base_name = os.path.splitext(os.path.basename(ofn_path))[0]
        if is_shacl_or_alignment_ttl(ofn_path):
            continue
        ...
        temp_g = Graph()
        try:
            parse_ofn_file(temp_g, ofn_path)
        except Exception as e:
            log.error("Error parsing OFN file %s (%s)", ofn_path, str(e))
            sys.exit(2)
    ...
    for ont_name, ont in ontology_info.items():
        temp_g = Graph()
        parse_ofn_file(temp_g, ont["file"])
        direct_imports = []
        for ont_iri in temp_g.subjects(RDF.type, OWL.Ontology):
    ...
    try:
        update_mkdocs_nav(mkdocs_path, ontology_info, global_all_classes, errors,
                          class_to_onts, ontology_info, ofn_files)
    except Exception as e:
